...
async def hyvaksyn(update: Update, context: CbCtx):
    ...
    if update.message.text == "Kyllä":
        ...
        await context.bot.send_message(
            ...
            (
                "Onneksi olkoon! Sinut on nyt lisätty käyttäjäksi. Kirjoittamalla"
                " /piikki_ohje näet mitä kaikkea sähköisellä piikillä voi tehdä."
            ),
...
async def lisaa(update: Update, context: CbCtx):
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            "Antamasi luku ei kelpaa. Lisääminen keskeytetty.",
            reply_markup=ReplyKeyboardRemove(),
        )
        return ConversationHandler.END
    user = update.effective_user.id
    ...
    await context.bot.send_message(
        update.message.chat.id,
        "Saldon lisääminen onnistui. Saldosi on nyt {:.2f}€".format(saldo / 100),
        reply_markup=ReplyKeyboardRemove(),
    )
    return ConversationHandler.END
...
async def poista(update: Update, context: CbCtx):
    ...
    if update.message.text == "Kyllä":
        ...
        await context.bot.send_message(
            update.message.chat.id,
            "Tapahtuma poistettu. Tilisi saldo on {:.2f}€.".format(saldo / 100),
            reply_markup=ReplyKeyboardRemove(),
        )
    else:
        await context.bot.send_message(
...
poisto_handler = ConversationHandler(
    ...
    fallbacks=[
        ...
        MessageHandler(filters.ALL, tuntematon),
    ],
    allow_reentry=True,
)
...
register_handler = ConversationHandler(
    ...
    states={HYVAKSYN: [MessageHandler(filters.ALL, hyvaksyn)]},
    fallbacks=[
        CommandHandler("lopeta", lopeta),
        MessageHandler(filters.ALL, tuntematon),
    ],
